...
class RecentPostsPlus extends WP_Widget {
    ...
    private $default_config = array(
        ...
        <div class="news-item">
            <div class="news_image {THUMB}">
                <a href="{PERMALINK}" class="news-img no_thickbox latest_posts">{THUMBNAIL}</a>
            </div>
            <div class="post_container {THUMB}">
                <h3>
                    <a href="{PERMALINK}">{TITLE}</a>
                </h3>
                {EXCERPT}
                <span class="post-time icon-clock paint-area paint-area--text">{POST_TIME}</span>
    ...
    function __construct() {
        $widget_ops = array(
            ...
            'description' => __('The most recent posts on your site. Includes advanced options.', 'functionality-for-boron-theme')
        );
        parent::__construct('recent-posts-plus', __('Boron - Recent Posts Plus', 'functionality-for-boron-theme'), $widget_ops);
    }
    ...
    function parse_output($instance) {
        ...
        if( $the_query->have_posts() ) {
            ...
            while ( $the_query->have_posts() ) { $the_query->the_post();
                ...
                if($include_post_thumbnail == "false" || get_the_post_thumbnail($ID, 'boron-small-thumbnail') == '' ) {
                    $POST_THUMBNAIL = '';
                    $extra_class = "no_thumb";
                } else {
                    $extra_class = "";
                    if ( $post_thumbnail_width ==  '' || $post_thumbnail_height == '' ) {
                        $POST_THUMBNAIL = get_the_post_thumbnail($ID, 'boron-small-thumbnail');
                    } else {
                ...
                foreach ($post_categories as $value) {
                    ...
                    $categories .= '<a href="' . $category_link . '">' . $value->name . '</a>, ';
                }
                ...
                $widget_ouput_template_params = array(
                    ...
                    '{POST_TIME}'     => human_time_diff(get_the_time('U',$ID),current_time('timestamp')) .  ' ago',
    ...
    function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'functionality-for-boron-theme'); ?></label>
            <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('Number of posts to show:', 'functionality-for-boron-theme'); ?></label>
        ...
        <p>
            <?php if( current_theme_supports('post-thumbnails') ): ?>
                ...
                <label for="<?php echo $this->get_field_id('include_post_thumbnail'); ?>"><?php _e('Include post thumbnail', 'functionality-for-boron-theme'); ?></label><br>
            <?php endif; ?>
            ...
            <label for="<?php echo $this->get_field_id('include_post_excerpt'); ?>"><?php _e('Include post excerpt', 'functionality-for-boron-theme'); ?></label><br>
            ...
            <label for="<?php echo $this->get_field_id('show_expert_options'); ?>"><?php _e('Show expert options', 'functionality-for-boron-theme'); ?></label>
        </p>

        <div class="rpp_expert-panel" style="display:none; margin-top: 10px">

            <p>
                <label for="<?php echo $this->get_field_id('truncate_post_title'); ?>"><?php _e('Limit post title:', 'functionality-for-boron-theme'); ?></label>
                ...
                <label for="<?php echo $this->get_field_id('truncate_post_excerpt'); ?>"><?php _e('Limit post excerpt:', 'functionality-for-boron-theme'); ?></label>
                ...
                <label for="<?php echo $this->get_field_id('truncate_elipsis'); ?>"><?php _e('Limit ellipsis:', 'functionality-for-boron-theme'); ?></label>
                ...
                <label for="<?php echo $this->get_field_id('post_date_format'); ?>"><?php _e('Post date format:', 'functionality-for-boron-theme'); ?></label> <a title="View Documentation" target="_blank" href="http://www.pjgalbraith.com/2011/08/recent-posts-plus/">(?)</a>
            ...
            <?php if( current_theme_supports('post-thumbnails') ): ?>
                <p>
                    <label for="<?php echo $this->get_field_id('post_thumbnail_width'); ?>"><?php _e('Thumbnail size (WxH):', 'functionality-for-boron-theme'); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('wp_query_options'); ?>"><?php _e('WP_Query Options', 'functionality-for-boron-theme'); ?></label> <a title="View Documentation" target="_blank" href="http://www.pjgalbraith.com/2011/08/recent-posts-plus/">(?)</a><br />
            ...
            <p>
                <label for="<?php echo $this->get_field_id('widget_output_template'); ?>"><?php _e('Widget Output Template', 'functionality-for-boron-theme'); ?></label> <a title="View Documentation" target="_blank" href="http://www.pjgalbraith.com/2011/08/recent-posts-plus/">(?)</a><br />
    ...
    static function rpp_form_script($hook_suffix) {
        if($hook_suffix == 'widgets.php') {
            /*wp_enqueue_script(
                'rpp_widget_script',
                plugins_url('master.js', __FILE__),
                array('jquery')
            );*/
        }
    }
